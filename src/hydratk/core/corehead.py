...
class CoreHead(EventHandler, Debugger, Profiler):
    ...
    _thr              = []; '''Core thread pool'''
    _trn              = None; '''Translator instance'''
    _pid_file         = None;
    _option           = {};
    _option_param     = {};    
    _app_service      = []; '''Application services pool'''  
    ...
    def c_worker(self, i, status, action_status, pipe_conn, is_alive_check): 
        ...
        self._reg_self_signal_hooks(); '''updating signal hooks to work properly'''
    ...
    def _load_base_config(self):                       
        if (os.path.exists(self._config_file)):            
            try:
                self.dmsg('htk_on_debug_info', 'Trying to loaded config ' + self._config_file, self.fromhere());
            ...
            try:
                if 'enabled' not in ext_cfg:
                    ...
                    if (ext_cfg['enabled'] == 1):
                        ...
                        #ex = self._internal_extension_module_import(ext_full_path);
                        print('Trying to import %s' % (ext_full_path));
    ...
    def _reg_self_command_hooks(self):        
        ...
        self.register_command_hook(hook);  
    ...
    def _set_default_cli_params(self):
        ...
        for opt in commands.long_opts:
            ...
            else: print('Missing option %s definition, languague %s' % (cmd, self._language))
    ...
    def _reg_self_event_hooks(self):
        ...
        hook = [
                {'event' : 'htk_on_error', 'callback' : self._eh_h_on_error, 'unpack_args' : True},        
                {'event' : 'htk_on_warning', 'callback' : self._eh_h_on_warning, 'unpack_args' : True}, 
                {'event' : 'htk_on_debug_info', 'callback' : self._eh_h_on_debug_info, 'unpack_args' : True},
                {'event' : 'htk_on_got_cmd_options', 'callback' : self._eh_h_on_got_cmd_options },
                {'event' : 'htk_on_extension_error', 'callback' : self._eh_h_on_extension_error, 'unpack_args' : True},
                {'event' : 'htk_on_extension_warning', 'callback' : self._eh_h_on_extension_warning, 'unpack_args' : True},
                {'event' : 'htk_on_uncaught_exception', 'callback' : self._eh_h_on_exception, 'unpack_args' : True}
            ];
        ...
        if (len(sys.argv) > 1):
            ...
            if options != False:                
                for opt_name, opt_value in options['options']:
                    ...
                    if opt_name == '-d' or opt_name == '--debug':
                        self._debug_level = int(opt_value);                
            ...
            self._command = self._default_command;
        self.fire_event(event.Event('h_before_cmd_options'));      
