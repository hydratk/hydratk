...
class EventHandler:
    ...
    def _eh_h_on_got_cmd_options(self, oevent):
        self.apply_command_options(); 
    
    def _eh_h_on_debug_info(self, oevent, *args):
        self.dout(*args);       
    
    def _eh_h_on_warning(self, oevent, *args):
        self.wout(*args);
    
    def _eh_h_on_extension_warning(self, oevent, *args):
        self.wout(*args); 
        
    def _eh_h_on_error(self, oevent, *args):
        self.errout(*args);
        
    def _eh_h_on_exception(self, oevent, *args):
        self.exout(*args);
    
    def _eh_h_on_extension_error(self, oevent, *args):
        self.errout(*args);
            
    def _ec_sig_handler(self, oevent, signum):
        signal = hsignal.sigint2string[signum] if signum in hsignal.sigint2string else signum; 
        self.dmsg('h_debug_info', self._trn.msg('htk_sig_recv', signal), self.fromhere());    
