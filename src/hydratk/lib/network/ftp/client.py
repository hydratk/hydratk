...
class FTPClient:
    ...
    def __init__(self, protocol='FTP', verbose=False):
        ...
        Args:
           protocol (str):  FTP protocol, FTP|FTPS|SFTP|TFTP          
        ...
        elif (self._protocol == 'FTPS'):
            self._client = ftplib.FTP_TLS()              
        elif (self._protocol not in ('SFTP', 'TFTP')):
            self._mh.dmsg('htk_on_error', self._mh._trn.msg('htk_ftp_unknown_protocol', self._protocol), self._mh.fromhere())
            return None
                         
        self.verbose = verbose 
        if (self.verbose):
            if (self._protocol in ('FTP', 'FTPS')):  
                self._client.set_debuglevel(2) 
            elif (self._protocol == 'SFTP'):                
                logging.getLogger('paramiko').setLevel(logging.DEBUG)
            elif (self._protocol == 'TFTP'):
    ...
    def connect(self, host, port=None, user=None, passw=None, path='/'):
        ...
        try:            
            ...
            if (ev.will_run_default()):    
                if (self._protocol in ('FTP', 'FTPS')):
                    self._client.connect(host, self._port)
                elif (self._protocol == 'SFTP'):                
                    t = paramiko.Transport((host, self._port))                                 
                elif (self._protocol == 'TFTP'):
                    self._client = tftpy.TftpClient(host, self._port)                  
                ...
                if (path != None and self._protocol in ('FTP', 'FTPS', 'SFTP')):
    ...
    def disconnect(self):
        ...
        try:
            ...
            if (self._protocol in ('FTP', 'FTPS')):                                 
                ...
                self._mh.dmsg('htk_on_error', self._mh._trn.msg('htk_ftp_unknown_method', self._protocol), self._mh.fromhere())
                return False
    ...
    def list_dir(self):
        ...
        try: 
            ...
            if (self._protocol in ('FTP', 'FTPS')):
                ...
                self._mh.dmsg('htk_on_error', self._mh._trn.msg('htp_ftp_unknown_method', self._protocol), self._mh.fromhere())
                return None            
